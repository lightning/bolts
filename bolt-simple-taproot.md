...
a regular tapscript spend path rather than a keyspend MuSig2 path. These
Schnorr signatures use BIP-340 deterministic nonce derivation with zero
`auxrand` (32 zero bytes). Implementations must use the same nonce derivation
to reproduce matching signatures — passing no auxiliary randomness (or
explicitly zero bytes) to the BIP-340 `Sign` algorithm.
...
### Zero-fee commitments test vectors

Test vectors [are provided](./bolt03/taproot-zero-fee-commitments-test.json) that detail commitment
and HTLC transactions created using `taproot_zero_fee_commitments` in various scenarios.
All parameters are provided to ensure that implementations generate exactly the same signed transactions.
