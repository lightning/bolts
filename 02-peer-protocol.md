...
Note that there is an additional issue with timed out (offered) HTLCs:
after an additional `to-self-delay` blocks, they can be redeemed by
the counterparty even without the revocation key (see
[BOLT #3 - Received HTLC Outputs](03-transactions.md#ereceived-htlc-outputs)).

Thus a node MUST estimate the deadline for timeout for each HTLC it
offers, which MUST BE less than the `cltv-expiry` plus the node's own
`to-self-delay`, and MUST fail the channel if its own current
commitment transaction contains the HTLC past this deadline.
...
MUST fail an HTLC which has timed out.
...
previous commitment transaction, and MUST set `next-per-commitment-point` to the values for its next commitment transaction.
