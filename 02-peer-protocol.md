...
The sending node:
  ...
  - if `option_splice` was negotiated:
    - MUST set `your_last_funding_locked_txid` to the txid of the last `splice_locked` it received.
    - if a splice transaction reached acceptable depth while disconnected:
      - MUST set `my_current_funding_locked_txid` to the txid of the latest such transaction.
