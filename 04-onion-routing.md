...
The node MUST abort processing and report a route failure if it receives a duplicated routing info which it has already forwarded or redeemed locally.  This prevents a node on the route from retrying a payment multiple times and attempting to track its progress by traffic analysis.  Note that this could be done using a log of previous shared secrets or HMACs, which can be forgotten once that HTLC would not be accepted anyway (eg. once `outgoing_cltv_value` has passed).  Such a log may use a probabilist data structure, but MUST rate-limit commitments as necessary to constrain the worst-case storage requirements or false positives of this log.
