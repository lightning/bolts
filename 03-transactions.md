...
* txout count: 0, 1 or 2
   ...
   * `txout` script: as specified in that node's `final_scriptpubkey` in its `open_channel` or `accept_channel` message, or 
   in its `scriptpubkey` in its `shutdown` message if no script was specified `open_channel` or `accept_channel`.
...
uses the local node's `delayed_payment_basepoint`, the `local_htlckey` use the remote node's `htlc_payment_basepoint` and the
